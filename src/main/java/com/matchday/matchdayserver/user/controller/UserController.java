...
public class UserController {
  ...
  @Operation(summary = "유저 프로필 이미지 read presigned URL 조회", description = "user 프로필 업로드에서 반환 받은 파일명(key)을 넘겨주면 read용 URL을 반환합니다")
  @GetMapping("/{userId}/profile/read-url")
  public ApiResponse<S3PresignedResponse> getProfileReadUrl(
      @PathVariable Long userId,
      @RequestParam String key
  ) {
    return ApiResponse.ok(s3PresignedManager.generateReadUrl(userId,key));
  }
